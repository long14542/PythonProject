...
class InsertWindow:
    def __init__(self):
        ...
        tkinter.Label(self.window, text = "Amount of water (liter)",  width = 25).grid(pady = 5, column = 1, row = 11)
    ...
    def Insert(self):
        ...
        if (self.test == "SUCCESS"):
            self.database.Insert(self.idEntry.get(), self.fNameEntry.get(), self.lNameEntry.get(), self.dobBox.get(), self.mobBox.get(), self.yobBox.get(), self.genderBox.get(), self.addressEntry.get(), self.phoneEntry.get(), self.emailEntry.get(), self.wateramountEntry.get())
